...
class Starlineresultcontroller extends MY_AdminController {
    ...
    public function decleareResult()
	{
		...
		if($this->session->userdata('adminlogin') == 1 && $this->session->userdata('admin_type') == 1)
		{
			...
			$this->data['master_menu'] = 'starline_game_name';
			$where=array('status'=>1);
			$by='open_time_sort';
			$this->data['game_result']=$this->Adminamodel->get_data_latest_where_asc($this->tb35,$by,$where);
	
          	$this->data['starlineResultHistoryListTableLoadFlag'] = 1;
			$this->middle = 'admin/a2'; 
...

	{
		...
		$result = $this->Adminamodel->get_data($this->tb36,$where);
		$data['game_id']='';
		$data['open_number']='';
		$data['close_number']='';
	...
	public function saveOpenData()
	{
		...
		$where=array('game_id'=>$game_id,'result_date'=>$result_dec_date);	
		$result = $this->Adminamodel->get_data($this->tb36,$where);								$where=array('game_id'=>$game_id,'open_decleare_status'=>1,'result_date'=>$result_dec_date);			$game_dec_result = $this->Adminamodel->get_data($this->tb36,$where);		if(count($game_dec_result)>0){						$data['status'] = "error"; 			$data['msg'] = $this->volanlib->error("Game result already Declared");			echo json_encode($data); die;		}
		
		if(count($result)<1)
		{

			$this->Adminbmodel->insertData($insert_data,$this->tb36);
...

		{
			//$where=array('id'=>$id);
			$where=array('game_id'=>$game_id,'result_date'=>$result_dec_date);	
			$this->Admincmodel->update_where($this->tb36,$insert_data,$where);
			$data['status'] = "success"; 
	...
	public function decleareOpenData()
	{
		...
		{						$where=array('game_id'=>$game_id,'open_decleare_status'=>1,'result_date'=>$result_dec_date);				$game_dec_result = $this->Adminamodel->get_data($this->tb36,$where);			if(count($game_dec_result)>0){								$data['status'] = "error"; 				$data['msg'] = $this->volanlib->error("Game result already Declared");				echo json_encode($data); die;			}
			foreach($result as $rs)
			{
			...
			$game_name_result = $this->Adminamodel->get_data($this->tb35,$where);
		...
		//echo "<pre>";print_r($result);die;
			...
			foreach($result as $rs)
			{
				...
				if($rs->pana=='Single Digit')
				{
					...
					if($win_number==$rs->digits)
					{
						...
						//$msg='Amount won in '.$rs->game_name.' '.$rs->pana.'  Bid Number- '.$rs->bid_tx_id;
						...
						$this->Admincmodel->update_where($this->tb37,$up_data,$where);
			...
			$this->Admincmodel->update_where($this->tb36,$up_data,$where);
	...
	{		
		...
		else
		{
			foreach($result as $rs)
			{
				...
				$this->Admindmodel->delete($this->tb36,$where_array);
	...
	public function getOpenWinnerList()
	{
		...
		$columns=$this->tb35.".game_name,".$this->tb3.".user_id,pana,session,digits,closedigits,points,bid_id,bid_tx_id,pay_status,user_name";
		$by = 'bid_id';
